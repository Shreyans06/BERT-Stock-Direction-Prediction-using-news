...
def train(args , file_name , model_name , model , loss_fn , train_dataloader , device , optimizer , scheduler , lr_scheduler , fold , val_dataloader = None , epochs = 4 , evaluation=True):
    """
    Train a BertClassifier model
    """
    print("Start Training..\n")
    if not os.path.exists(f"./models/{file_name}_{model_name}_{args.summary_type}_{args.column}_{args.tkl}_{args.batch_size}_{args.lr}"):
        os.makedirs(f"./models/{file_name}_{model_name}_{args.summary_type}_{args.column}_{args.tkl}_{args.batch_size}_{args.lr}")
    if not os.path.exists(f"./metrics/{file_name}_{model_name}_{args.summary_type}_{args.column}_{args.tkl}_{args.batch_size}_{args.lr}"):    
        os.makedirs(f"./metrics/{file_name}_{model_name}_{args.summary_type}_{args.column}_{args.tkl}_{args.batch_size}_{args.lr}")
    ...
    for epoch in range(epochs):
        ...
        for step , batch in enumerate(train_dataloader ):
            ...
            training_accuracy = (train_preds == labels).cpu().numpy().mean() * 100
        ...
        if evaluation == True:
            ...
            if val_loss < best_val_loss:
                ...
                torch.save({
                    ...
                    }, f"./models/{file_name}_{model_name}_{args.summary_type}_{args.column}_{args.tkl}_{args.batch_size}_{args.lr}/{fold}.pt")
    ...
    try:
        torch.save({'val_accuracy': val_accuracy_epochs , 'val_loss' : val_loss_epochs , 'train_loss' : train_loss_epochs, 'train_accuracy' : train_accuracy_epochs }, 
                   f"./metrics/{file_name}_{model_name}_{args.summary_type}_{args.column}_{args.tkl}_{args.batch_size}_{args.lr}/{fold}.pt")
    except:
        print("Save failed")
